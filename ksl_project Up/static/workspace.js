...
class KSLWorkspace {
    ...
    async startTraining() {
        ...
        try {
            ...
            const finalAccuracy = document.getElementById('valAccuracy').textContent;
    ...
    updateTopPredictions(predictions) {
        ...
        predictions.slice(0, 5).forEach((pred, index) => {
            ...
            item.innerHTML = `
                <span class="prediction-rank">${rank}위:</span>
    ...
    async loadModelManagementList() {
        try {
            ...
            if (data.success && data.models.length > 0) {
                data.models.forEach(model => {
                    ...
                    item.innerHTML = `
                        <div class="model-info">
                            ...
                            <div class="model-details">
                                <div class="model-detail-item">
                                    <span>📊</span>
                                    <span>정확도: ${model.accuracy.toFixed(1)}%</span>
                                </div>
                                <div class="model-detail-item">
                                    <span>🎯</span>
                                    <span>제스처: ${model.gestures}종류</span>
                                </div>
                                <div class="model-detail-item">
                                    <span>📈</span>
                                    <span>샘플: ${model.samples}개</span>
                                </div>
                                <div class="model-detail-item">
                                    <span>⏱️</span>
                        ...
                        <div class="model-actions">
                            <button class="btn-small btn-rename" data-model-name="${model.name}">
                                ✏️ 이름 변경
                            </button>
                            <button class="btn-small btn-delete" data-model-name="${model.name}">
                                🗑️ 삭제
                            </button>
                        </div>
                    `;
                ...
                container.innerHTML = '<p class="text-muted" style="text-align: center; padding: 40px;">저장된 모델이 없습니다. 먼저 모델을 학습해주세요.</p>';
            ...
            console.error('모델 목록 로드 실패:', error);
            const container = document.getElementById('modelManagementList');
            container.innerHTML = '<p class="text-muted" style="color: #f56565;">모델 목록을 불러오는데 실패했습니다.</p>';
        }
    }
